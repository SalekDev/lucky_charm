...
public class LuckyCharmPerks {
    ...
    public static final Text FORTUNED_TRAVELLER_TEXT = Text.translatable(
                    Util.createTranslationKey("perk", Identifier.of(LuckyCharm.MOD_ID, "fortuned_traveller")))
            .formatted(Formatting.DARK_GREEN);
    public static final Text FORTUNED_TRAVELLER_ADDITIONS_SLOT_DESCRIPTION_TEXT = Text.translatable(
            Util.createTranslationKey("item", Identifier.of(LuckyCharm.MOD_ID, "smithing_template.fortuned_traveller.additions_slot_description"))
    );
    public static void bootstrap(Registerable<AttributeEquipmentPerk> registry) {
        AttributeEquipmentPerk fortunedTravellerPerk = new AttributeEquipmentPerk(new AttributeEquipmentPerk.AttributeModifier(EntityAttributes.LUCK,
                "fortuned_traveller", 1.0, EntityAttributeModifier.Operation.ADD_VALUE), FORTUNED_TRAVELLER_TEXT,true);
    ...
    public static Optional<RegistryEntry.Reference<AttributeEquipmentPerk>> get(RegistryWrapper.WrapperLookup lookup, RegistryKey<AttributeEquipmentPerk> perkKey) {
        return lookup.getOrThrow(LuckyCharmPerks.PERKS_KEY).getOptional(perkKey);
    }
